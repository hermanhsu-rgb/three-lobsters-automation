...
platforms:
  feishu:
    enabled: true
    extra:
      app_id: <小结巴自己的app_id>        # ⚠️ 必须用小结巴自己的！
      app_secret: <小结巴自己的app_secret>  # ⚠️ 必须用小结巴自己的！
      dm_policy: open        # 私聊开放
...
|------|-----|------|
| `enabled` | `true` | 启用飞书平台 |
| `app_id` | 小结巴自己的 | 飞书开放平台应用ID |
| `app_secret` | 小结巴自己的 | 飞书开放平台应用密钥 |
| `dm_policy` | `open` | 私聊消息开放 |
| `group_policy` | `open` | 群消息开放（关键！） |
| `require_mention` | `false` | 不需要@也能触发 |
| `bot_trigger_names` | `["小结巴", "xiaojieba"]` | 触发关键词 |
...
platforms:
  feishu:
    enabled: true
    extra:
      app_id: cli_xxxxxxxxxxxx       # 替换成小结巴自己的
      app_secret: xxxxxxxxxxxxxxxx   # 替换成小结巴自己的
      dm_policy: open
      group_policy: open
...
**文档版本**: 2026-05-02
**仓库**: github.com/hermanhsu-rgb/three-lobsters-automation
